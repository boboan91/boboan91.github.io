    ...
    <div style="display: flex;height: calc(100% - 47px);">
      ...
      >
        ...
        <flow-node-form
          ...
          @lineStyleChange="lineStyleChange"
  ...
  methods: {
    ...
    jsPlumbInit() {
      ...
      this.jsPlumb.ready(() => {
        ...
        this.jsPlumb.bind("click", (conn, originalEvent) => {
          ...
          getDataB().lineList.forEach((item, index) => {
            if (item.from == conn.sourceId && item.to == conn.targetId) {
              _this.$refs.nodeForm.lineDataInit(item);
            }
          });
        });
        // 连线
    ...
    loadEasyFlow() {
      ...
      for (var i = 0; i < this.data.lineList.length; i++) {
        ...
        let line = this.data.lineList[i];
        let linestyle = this.jsPlumb.connect(
          {
            source: line.from,
            target: line.to,
          },
        ...
        this.jsPlumb.registerConnectionType("example", {
          paintStyle: { stroke: line.color, strokeWidth: line.linewidth },
        });
        linestyle.setType("example");
      }
    ...
    //修改连线样式
    lineStyleChange(data, fn) {
      this.jsPlumb.registerConnectionType("customs", {
        paintStyle: {
          stroke: data.color,
     ...
     */
    deleteNode(nodeData) {
      this.$confirm("确定要删除节点" + nodeData.name + "?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
        ...
        .then(() => {
          ...
          this.$nextTick(function() {
            this.jsPlumb.removeAllEndpoints(nodeData.id);
          });
        })
        .catch(() => {});
      return true;
    ...
    dataReload(data) {
      ...
      this.$nextTick(() => {
        ...
        this.$nextTick(() => {
          ...
          this.$nextTick(() => {
            this.jsPlumbInit();
            this.$refs.nodeForm.nodeInit(this.data);
          });
        });
      });
    },
